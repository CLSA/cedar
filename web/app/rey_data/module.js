        ...
        return {
          ...
          controller: async function ($scope) {
            if (angular.isUndefined($scope.model))
              $scope.model = CnReyDataModelFactory.root;

            angular.extend($scope, {
              isComplete: false,
              isWorking: false,
              wordTypeaheadTemplateUrl: cenozoApp.getFileUrl(
                "cedar",
                  ...
                  if (proceed && angular.isString($scope.newWord)) {
                    ...
                    if ($scope.newWord.match(/^-+$/)) {
                      await CnModalMessageFactory.instance({
                        ...
                        message:
                          "You cannot use placeholders for the REY test.",
                        error: true,
                      }).show();
                      $scope.newWord = "";
                      proceed = false;
              ...
              patch: async function (property) {
                if ($scope.model.getEditEnabled()) {
                  ...
                  if ("language_id" == property) {
                    ...
                    data.language_id =
                      $scope.model.viewModel.record.language_id;
                  } else {
                    var variantProperty = property + "_rey_data_variant_id";
                    var wordValue =
                      $scope.model.viewModel.wordList.findByProperty(
                        "name",
                        property
                      ).value;
                  ...
                  if ("language_id" == property) {
                    try {
                      ...
                      await $scope.model.testEntryModel.viewModel.languageModel.listModel.onList(
                        true
                      );
                    } finally {
                    ...
                    $scope.model.viewModel.record[variantProperty] =
                      null == data[variantProperty]
                        ? ""
                        : data[variantProperty];
                  }
                }
              },
            });
            ...
            $scope.$watch(
              "model.testEntryModel.viewModel.languageIdList",
              function (list) {
                $scope.model.viewModel.onView();
                $scope.model.variantList.forEach((variant) => {
        ...
        var object = function (parentModel, root) {
          ...
          angular.extend(this, {
            ...
            submitIntrusion: async function (word) {
              ...
              if (
                angular.isObject(word) &&
                1 == newWordList.length &&
                word.word == newWordList[0]
              )
                newWordList = [word];

              await Promise.all(
                newWordList.map(async (word) => {
                  var text = angular.isString(word) ? word : word.word;
                  ...
                  var label = this.wordList.findByProperty(
                    "label",
                    text.ucWords()
                  );
                  if (label) {
                    ...
                    var variant = this.parentModel.variantList
                      .filter(
                        (obj) => obj.language_id == this.record.language_id
                      )
                    ...
                    if (variant) {
                      if (!variant.allowed) {
                        await CnModalMessageFactory.instance({
                          ...
                          message:
                            'You have selected the variant word "' +
                            text +
                        ...
                        var data = {};
                        data[variant.word + "_rey_data_variant_id"] =
                          variant.value;
                        await this.onPatch(data);
                        this.record[variant.word + "_rey_data_variant_id"] =
                          variant.value;
                        this.record[variant.word] = "";
                        this.wordList.findByProperty(
                          "name",
                          variant.word
                      ...
                      if (angular.isString(word)) {
                        ...
                        var response = await CnModalNewWordFactory.instance({
                          ...
                          languageIdRestrictList:
                            this.parentModel.testEntryModel.viewModel
                              .languageIdList,
                        }).show();
                        ...
                        if (null != response) {
                          ...
                          if (
                            this.intrusionList.some(
                              (intrusion) =>
                                intrusion.language_id == response &&
                                intrusion.word == word
                            )
                          ) {
                            alreadyExists = true;
                      ...
                      if (
                        alreadyExists ||
                        this.intrusionList.findByProperty("id", word.id)
                      ) {
                        await CnModalMessageFactory.instance({
                        ...
                        var data = {
                          add: angular.isDefined(word.id) ? word.id : word,
                        };

                        var response = await CnHttpFactory.instance({
                          path:
                            this.parentModel.getServiceResourcePath() + "/word",
                          data: data,
            ...
            deleteIntrusion: async function (wordRecord) {
              ...
              var index = this.intrusionList.findIndexByProperty(
                "id",
                wordRecord.id
              );
            ...
            onView: async function () {
              ...
              this.wordList.forEach((word) => {
                ...
                if (Number.isInteger(this.record[word.name]))
                  word.value = this.record[word.name];
            ...
            checkBeforeSubmit: async function () {
              ...
              this.wordList.forEach((word) => {
                ...
                if (this.record[property]) {
                  variantCount++;
                  var variant = this.parentModel.variantList.findByProperty(
                    "value",
                    this.record[property]
                  );
                  if (variant.variant_language_id == this.record.language_id)
                    languageCount++;
                }
              });
              ...
              if (0 == languageCount && 1 < variantCount) {
                var response = await CnModalConfirmFactory.instance({
                  ...
                  message:
                    "Are you sure that the REY test was administered in " +
                    this.record.language_name +
                    "?\n\n" +
                    "None of the selected variant words are in " +
                    this.record.language_name +
                    ", " +
                    "however the test is currently set to that language. " +
                    "If you believe the test was not administered in " +
                    this.record.language_name +
                    " " +
      ...
      function (CnBaseDataModelFactory, CnReyDataViewFactory, CnHttpFactory) {
        var object = function (root, testEntryModel) {
          ...
          this.getMetadata = async function () {
            ...
            var [wordResponse, reyDataVariantResponse, languageResponse] =
              await Promise.all([
            ...
            wordResponse.data.forEach((item) => {
              this.sisterList.push({
                ...
                word: item.word,
                sisterWordList:
                  null == item.sister_list ? [] : item.sister_list.split(","),
              });
            });

            // build a list of all primary, variant and sister words
            this.sisterList.forEach((sisterWord) => {
              sisterWord.sisterWordList.forEach((sister) =>
                this.fullWordList.push(sister)
              );
            ...
            languageResponse.data.forEach((item) => {
