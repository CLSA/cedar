  ...
  create: (module) => {
    ...
    module.addInputGroup("", {
      ...
      participant_status_type: {
        column: "participant_status_type.name",
        type: "string",
      },
      ...
      participant_language_id: {
        column: "participant.language_id",
        type: "hidden",
      },
        ...
        return {
          ...
          controller: async function ($scope) {
            if (angular.isUndefined($scope.model))
              $scope.model = CnTestEntryModelFactory.root;
            ...
            angular.extend($scope, {
              refresh: async function () {
                if ($scope.isComplete) {
                  ...
                  var type =
                    $scope.model.viewModel.record.data_type + "DataModel";
              ...
              onKeyboardShortcut: function (event) {
                if (
                  ...
                  if (null != action) {
                    var soundFile =
                      $scope.model.viewModel.soundFileList.findByProperty(
                        "active",
                        true
                      );

                    if (soundFile) {
                      var soundEl = document.getElementById(
                        "soundFile" + soundFile.id
                      );
                      // now determine what to do
                      if ("togglePlay" == action) {
                        if (soundEl.paused) soundEl.play();
                        else soundEl.pause();
              ...
              selectSoundFile: function (id) {
                ...
                $scope.model.viewModel.soundFileList.forEach((soundFile) => {
        ...
        var object = function (parentModel, root) {
          ...
          angular.extend(this, {
              ...
              if (!this.updatingAudioStatusList) {
                ...
                try {
                  var response = await CnHttpFactory.instance({
                    path:
                      "test_type/" + this.record.test_type_id + "/status_type",
                    data: {
                  ...
                  this.parentModel.metadata.columnList.audio_status_type_id.enumList =
                    angular.copy(enumList);
                  this.parentModel.metadata.columnList.participant_status_type_id.enumList =
                    angular.copy(enumList);
                  this.parentModel.metadata.columnList.admin_status_type_id.enumList =
                    angular.copy(enumList);

                  response.data.forEach((statusType) => {
                    if (this.record.audio_status_type_id == statusType.id)
                      audioFound = true;
                    else if (
                      this.record.participant_status_type_id == statusType.id
                    )
                      participantFound = true;
                    else if (this.record.admin_status_type_id == statusType.id)
                      adminFound = true;
                    this.parentModel.metadata.columnList[
                      statusType.category + "_status_type_id"
                    ].enumList.push({
                      value: statusType.id,
                  ...
                  if (!audioFound && this.record.audio_status_type_id) {
                    this.parentModel.metadata.columnList.audio_status_type_id.enumList.push(
                      {
                        value: this.record.audio_status_type_id,
                        name: this.record.audio_status_type,
                      }
                    );
                  }
                  ...
                  if (
                    !participantFound &&
                    this.record.participant_status_type_id
                  ) {
                    this.parentModel.metadata.columnList.participant_status_type_id.enumList.push(
                      {
                        value: this.record.participant_status_type_id,
                        name: this.record.participant_status_type,
                      }
                    );
                  }
                  ...
                  if (!adminFound && this.record.admin_status_type_id) {
                    this.parentModel.metadata.columnList.admin_status_type_id.enumList.push(
                      {
                        value: this.record.admin_status_type_id,
                        name: this.record.admin_status_type,
                      }
                    );
                  }
                } finally {
            ...
            onView: async function (force) {
              ...
              try {
                ...
                var response = await CnHttpFactory.instance({
                  ...
                  onError: function (error) {
                    ...
                    if (404 == error.status) {
                      console.warn(
                        "Problem loading sound files for " +
              ...
              this.soundFileList.forEach((soundFile, index) => {
                soundFile.active = 0 == index;
              });
            },
              ...
              if (checkNote) {
                ...
                if (
                  ...
                  var response = await CnModalTextFactory.instance({
                    ...
                    message:
                      "Please provide the reason you are changing the test entry's state:",
                    minLength: 10,
                  }).show();
                ...
                try {
                  ...
                  await CnHttpFactory.instance({
                    ...
                    onError: function (error) {
                      if (409 == error.status) {
                        var message =
                          "The test-entry cannot be submitted if it ";
                        message +=
                          "aft" == self.record.data_type ||
                          "fas" == self.record.data_type
                  ...
                  if ("assigned" != state && this.parentModel.isRole("typist"))
            ...
            otherStatusTypeSelected: function (type) {
              ...
              return (
                null != this.record[strProp] &&
                null != this.record[strProp].match("Other")
              );
            },
            patchStatus: async function (type) {
              ...
              if (this.parentModel.getStatusEditEnabled()) {
                ...
                data[idProp] =
                  "" == this.record[idProp] ? null : this.record[idProp];
                var statusType = this.parentModel.metadata.columnList[
                  idProp
                ].enumList.findByProperty("value", data[idProp]);
                if (
                  null == statusType ||
                  null == statusType.name.match("Other")
                )
                  this.record[otherProp] = null;
                data[otherProp] = this.record[otherProp];

                // also update the status_type string value
                this.record[strProp] =
                  null == statusType ? null : statusType.name;
            ...
            submit: async function () {
              ...
              if (
                (this.otherStatusTypeSelected("audio") &&
                  !this.record.audio_status_type_other) ||
                (this.otherStatusTypeSelected("participant") &&
                  !this.record.participant_status_type_other) ||
                (this.otherStatusTypeSelected("admin") &&
                  !this.record.admin_status_type_other)
              ) {
                await CnModalMessageFactory.instance({
                  title: "Cannot Submit",
                  message:
                    'The test-entry cannot be submitted because a status type of "Other" is selected but additional status ' +
                ...
                if ("aft" == this.record.data_type) {
                  ...
                  throw new Error(
                    'Invalid data type "' + this.record.data_type + '"'
                  );
                }
            ...
            defer: async function () {
              await this.setState("deferred", "typist");
            },
            returnToTypist: async function () {
              ...
              if (null == this.record.user_id) {
                var response = await CnModalSelectTypistFactory.instance({
                  message:
                    "Please select which typist this transcription should be re-assigned to:",
              ...
              try {
                await CnHttpFactory.instance({
                  ...
                  onError: function (error) {
                    ...
                    return 403 == error.status
                      ? self.parentModel.transitionToParentListState(
                          "transcription"
                        )
                ...
                if (null == this.record[columnName]) {
                  await this.parentModel.transitionToParentViewState(
                    "transcription",
                    "uid=" + this.record.transcription_uid
                  );
                } else {
                  await this.parentModel.transitionToViewState({
                    getIdentifier: function () {
                      return self.record[columnName];
                    },
                  });
                }
              } catch (error) {
            ...
            reset: async function () {
              await CnHttpFactory.instance({
                path: this.parentModel.getServiceResourcePath() + "?reset=1",
              }).patch();
            },
            close: async function () {
              if (this.parentModel.isRole("typist")) {
                try {
                  ...
                  await CnHttpFactory.instance({
                    path:
                      this.parentModel.getServiceResourcePath() + "?close=1",
                    onError: function (error) {
                      // ignore 403 errors since records may automatically be unassigned
                      if (403 != error.status)
                        return CnModalMessageFactory.httpError(error);
                    },
                  }).patch();
                } catch (error) {
                  // handled by onError above
            ...
            viewTranscription: async function () {
              return await $state.go("transcription.view", {
                identifier: "uid=" + this.record.transcription_uid,
              });
            },
            ...
            if (angular.isDefined(object.languageModel)) {
              object.languageModel.listModel.afterList(function () {
                if (!object.languageModel.listModel.chooseMode) {
                  ...
                  if (
                    !object.languageIdList.includes(
                      object.record.participant_language_id
                    )
                  )
                    object.record.participant_language_id =
                      object.languageIdList[0];
                }
              });
              ...
              object.languageModel.getChooseEnabled = function () {
                return (
                  object.languageModel.$$getChooseEnabled() &&
            ...
            angular.extend(object.parentModel, {
              ...
              getSubStatusEditEnabled: function (base) {
                return (
                  ...
                  (object[
                    object.record.data_type + "DataModel"
                  ].getEditEnabled() ||
                    object[
                      object.record.data_type + "DataModel"
                    ].getAddEnabled())
    ...
    cenozo.providers.factory("CnTestEntryModelFactory", [
      "CnBaseModelFactory",
      "CnTestEntryListFactory",
      "CnTestEntryViewFactory",
      "CnSession",
      "CnHttpFactory",
      "CnModalMessageFactory",
      function (
          ...
          async function init(object) {
            ...
            CnSession.setBreadcrumbTrail([
              ...
              },
              {
                title: object.uid,
                go: async function () {
                  await $state.go("transcription.view", {
                    identifier: "uid=" + object.uid,
                  });
                },
              },
              {
              ...
              },
              {
                ...
                go: async function () {
                  await $state.go("test_entry.view", $state.params);
                },
              },
              {
                title: "Notes",
