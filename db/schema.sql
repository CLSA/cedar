...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cenozo`.`jurisdiction_AFTER_INSERT`
AFTER INSERT ON `cenozo`.`jurisdiction`
FOR EACH ROW
BEGIN
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cenozo`.`jurisdiction_AFTER_UPDATE`
AFTER UPDATE ON `cenozo`.`jurisdiction`
FOR EACH ROW
BEGIN
  CALL update_participant_site_for_jurisdiction( NEW.id );
END$$
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cenozo`.`jurisdiction_BEFORE_DELETE`
BEFORE DELETE ON `cenozo`.`jurisdiction`
FOR EACH ROW
BEGIN
  DELETE FROM participant_site
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cenozo`.`region_site_AFTER_DELETE`
AFTER DELETE ON `cenozo`.`region_site`
FOR EACH ROW
BEGIN
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cenozo`.`region_site_AFTER_INSERT`
AFTER INSERT ON `cenozo`.`region_site`
FOR EACH ROW
BEGIN
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cenozo`.`region_site_AFTER_UPDATE`
AFTER UPDATE ON `cenozo`.`region_site`
FOR EACH ROW
BEGIN
  CALL update_participant_site_for_region_site( NEW.id );
END$$
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cenozo`.`region_site_BEFORE_DELETE`
BEFORE DELETE ON `cenozo`.`region_site`
FOR EACH ROW
BEGIN
  DELETE FROM participant_site
  WHERE site_id = OLD.site_id;
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cenozo`.`region_site_BEFORE_INSERT`
BEFORE INSERT ON `cenozo`.`region_site`
FOR EACH ROW
BEGIN
  SET @test = (
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cenozo`.`region_site_BEFORE_UPDATE`
BEFORE UPDATE ON `cenozo`.`region_site`
FOR EACH ROW
BEGIN
  SET @test = (
    SELECT COUNT(*) FROM region_site
