...
-- -----------------------------------------------------
-- Table `cedar`.`jurisdiction`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `cedar`.`jurisdiction` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `update_timestamp` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP(),
  `create_timestamp` TIMESTAMP NOT NULL DEFAULT '0000-00-00 00:00:00',
  `site_id` INT(10) UNSIGNED NOT NULL,
...
CREATE TABLE IF NOT EXISTS `cedar`.`premat_data` (
  ...
  CONSTRAINT `fk_premat_data_test_entry_id`
    ...
    ON UPDATE CASCADE)
ENGINE = InnoDB
AUTO_INCREMENT = 20372
DEFAULT CHARACTER SET = utf8;


-- -----------------------------------------------------
-- Table `cedar`.`region_site`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `cedar`.`region_site` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `update_timestamp` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP(),
  `create_timestamp` TIMESTAMP NOT NULL DEFAULT '0000-00-00 00:00:00' COMMENT 'Used to determine a participant\'s default site.',
  `site_id` INT(10) UNSIGNED NOT NULL,
...
CREATE TABLE IF NOT EXISTS `cenozo`.`alternate` (
  ...
  `decedent` TINYINT(1) NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS `cenozo`.`alternate_last_written_alternate_consent` (
  ...
  PRIMARY KEY (`alternate_id`, `alternate_consent_type_id`),
  INDEX `fk_alternate_consent_id` (`alternate_consent_id` ASC),
  INDEX `fk_alternate_last_written_alternate_consent_alternate_conse_idx` (`alternate_consent_type_id` ASC),
...
CREATE TABLE IF NOT EXISTS `cenozo`.`study` (
  ...
  CONSTRAINT `fk_study_consent_type_id`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_study_consent_identifier_id`
    FOREIGN KEY (`identifier_id`)
    REFERENCES `cenozo`.`identifier` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
-- -----------------------------------------------------
-- procedure import_rey_word
-- -----------------------------------------------------

DELIMITER $$
USE `cedar`$$
CREATE DEFINER=`patrick`@`localhost` PROCEDURE `import_rey_word`( rank INT, word VARCHAR(7) )
BEGIN
...
DELIMITER $$
USE `cedar`$$
CREATE DEFINER=`patrick`@`localhost` PROCEDURE `update_participant_sound_file_total`(IN proc_participant_id INT(10) UNSIGNED)
BEGIN
...
DELIMITER $$
USE `cedar`$$
CREATE DEFINER=`patrick`@`localhost` PROCEDURE `update_test_entry_has_word`(IN proc_test_entry_id INT(10) UNSIGNED)
BEGIN
...
DELIMITER $$
USE `cedar`$$
CREATE DEFINER=`patrick`@`localhost` PROCEDURE `update_transcription_has_language`(IN proc_transcription_id INT(10) UNSIGNED)
BEGIN
...
DELIMITER $$
USE `cedar`$$
CREATE DEFINER=`patrick`@`localhost` PROCEDURE `update_transcription_has_user`(IN proc_transcription_id INT(10) UNSIGNED)
BEGIN
...
DELIMITER $$
USE `cedar`$$
CREATE DEFINER=`patrick`@`localhost` PROCEDURE `update_word_test_type_total`(IN proc_word_id INT(10) UNSIGNED, IN proc_data_type ENUM('aft','fas','mat','premat','rey'))
BEGIN
...
BEGIN
  ...
  AND application_has_cohort.application_id = 0
...
TRIGGER `cedar`.`aft_data_AFTER_DELETE`
AFTER DELETE ON `cedar`.`aft_data`
FOR EACH ROW
BEGIN
  SET @test = (
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cedar`.`aft_data_AFTER_INSERT`
AFTER INSERT ON `cedar`.`aft_data`
FOR EACH ROW
BEGIN
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cedar`.`aft_data_AFTER_UPDATE`
AFTER UPDATE ON `cedar`.`aft_data`
FOR EACH ROW
BEGIN
  CALL update_test_entry_has_word( NEW.test_entry_id );
END$$
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cedar`.`fas_data_AFTER_DELETE`
AFTER DELETE ON `cedar`.`fas_data`
FOR EACH ROW
BEGIN
  SET @test = (
    SELECT SUM( total ) FROM (
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cedar`.`fas_data_AFTER_INSERT`
AFTER INSERT ON `cedar`.`fas_data`
FOR EACH ROW
BEGIN
  CALL update_test_entry_has_word( NEW.test_entry_id );
END$$
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cedar`.`fas_data_AFTER_UPDATE`
AFTER UPDATE ON `cedar`.`fas_data`
FOR EACH ROW
BEGIN
  CALL update_test_entry_has_word( NEW.test_entry_id );
END$$
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cedar`.`jurisdiction_AFTER_INSERT`
AFTER INSERT ON `cedar`.`jurisdiction`
FOR EACH ROW
BEGIN
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cedar`.`jurisdiction_AFTER_UPDATE`
AFTER UPDATE ON `cedar`.`jurisdiction`
FOR EACH ROW
BEGIN
  CALL update_participant_site_for_jurisdiction( NEW.id );
END$$
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cedar`.`jurisdiction_BEFORE_DELETE`
BEFORE DELETE ON `cedar`.`jurisdiction`
FOR EACH ROW
BEGIN
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cenozo`.`jurisdiction_AFTER_DELETE`
AFTER DELETE ON `cenozo`.`jurisdiction`
FOR EACH ROW
BEGIN
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cedar`.`region_site_AFTER_INSERT`
AFTER INSERT ON `cedar`.`region_site`
FOR EACH ROW
BEGIN
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cedar`.`region_site_AFTER_UPDATE`
AFTER UPDATE ON `cedar`.`region_site`
FOR EACH ROW
BEGIN
  CALL update_participant_site_for_region_site( NEW.id );
END$$
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cedar`.`region_site_BEFORE_DELETE`
BEFORE DELETE ON `cedar`.`region_site`
FOR EACH ROW
BEGIN
  DELETE FROM participant_site
  WHERE site_id = OLD.site_id;
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cedar`.`region_site_AFTER_DELETE`
AFTER DELETE ON `cedar`.`region_site`
FOR EACH ROW
BEGIN
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cedar`.`rey_data_has_word_AFTER_DELETE`
AFTER DELETE ON `cedar`.`rey_data_has_word`
FOR EACH ROW
BEGIN
  SET @test = (
    SELECT SUM( total ) FROM (
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cedar`.`rey_data_has_word_AFTER_INSERT`
AFTER INSERT ON `cedar`.`rey_data_has_word`
FOR EACH ROW
BEGIN
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cedar`.`rey_data_has_word_AFTER_UPDATE`
AFTER UPDATE ON `cedar`.`rey_data_has_word`
FOR EACH ROW
BEGIN
  SET @test_entry_id = ( SELECT test_entry_id FROM rey_data WHERE id = NEW.rey_data_id );
  CALL update_test_entry_has_word( @test_entry_id );
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cedar`.`sound_file_AFTER_DELETE`
AFTER DELETE ON `cedar`.`sound_file`
FOR EACH ROW
BEGIN
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cedar`.`sound_file_AFTER_INSERT`
AFTER INSERT ON `cedar`.`sound_file`
FOR EACH ROW
BEGIN
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cedar`.`sound_file_AFTER_UPDATE`
AFTER UPDATE ON `cedar`.`sound_file`
FOR EACH ROW
BEGIN
  CALL update_participant_sound_file_total( NEW.participant_id );
END$$
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cedar`.`test_entry_AFTER_UPDATE`
AFTER UPDATE ON `cedar`.`test_entry`
FOR EACH ROW
BEGIN
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cedar`.`test_entry_activity_AFTER_DELETE`
AFTER DELETE ON `cedar`.`test_entry_activity`
FOR EACH ROW
BEGIN
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cedar`.`test_entry_activity_AFTER_INSERT`
AFTER INSERT ON `cedar`.`test_entry_activity`
FOR EACH ROW
BEGIN
  SET @transcription_id = (
    SELECT transcription_id
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cedar`.`test_entry_activity_AFTER_UPDATE`
AFTER UPDATE ON `cedar`.`test_entry_activity`
FOR EACH ROW
BEGIN
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cedar`.`test_entry_has_language_AFTER_DELETE`
AFTER DELETE ON `cedar`.`test_entry_has_language`
FOR EACH ROW
BEGIN
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cedar`.`test_entry_has_language_AFTER_INSERT`
AFTER INSERT ON `cedar`.`test_entry_has_language`
FOR EACH ROW
BEGIN
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cedar`.`test_entry_has_language_AFTER_UPDATE`
AFTER UPDATE ON `cedar`.`test_entry_has_language`
FOR EACH ROW
BEGIN
  SET @transcription_id = ( SELECT transcription_id FROM test_entry WHERE id = NEW.test_entry_id );
  CALL update_transcription_has_language( @transcription_id );
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cedar`.`test_entry_has_word_AFTER_DELETE`
AFTER DELETE ON `cedar`.`test_entry_has_word`
FOR EACH ROW
BEGIN
  SET @data_type = (
    SELECT test_type.data_type
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cedar`.`test_entry_has_word_AFTER_INSERT`
AFTER INSERT ON `cedar`.`test_entry_has_word`
FOR EACH ROW
BEGIN
  SET @data_type = (
    SELECT test_type.data_type
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cedar`.`test_entry_has_word_AFTER_UPDATE`
AFTER UPDATE ON `cedar`.`test_entry_has_word`
FOR EACH ROW
BEGIN
  SET @data_type = (
    SELECT test_type.data_type
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cedar`.`transcription_AFTER_UPDATE`
AFTER UPDATE ON `cedar`.`transcription`
FOR EACH ROW
BEGIN
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cedar`.`word_AFTER_INSERT`
AFTER INSERT ON `cedar`.`word`
FOR EACH ROW
BEGIN
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cedar`.`word_BEFORE_UPDATE`
BEFORE UPDATE ON `cedar`.`word`
FOR EACH ROW
BEGIN
...
USE `cenozo`$$
CREATE DEFINER = CURRENT_USER TRIGGER `cenozo`.`alternate_consent_type_AFTER_INSERT` AFTER INSERT ON `cenozo`.`alternate_consent_type` FOR EACH ROW
BEGIN
