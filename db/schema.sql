...
-- -----------------------------------------------------
-- Table `cedar`.`custom_report`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `cedar`.`custom_report` (
...
-- -----------------------------------------------------
-- Table `cedar`.`role_has_custom_report`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `cedar`.`role_has_custom_report` (
  `role_id` INT(10) UNSIGNED NOT NULL,
  ...
  CONSTRAINT `fk_role_has_custom_report_custom_report_id`
    ...
    REFERENCES `cedar`.`custom_report` (`id`)
    ON DELETE CASCADE
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8
...
-- -----------------------------------------------------
-- Table `cedar`.`transcription_event_type`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `cedar`.`transcription_event_type` (
...
CREATE
DEFINER=`patrick`@`localhost`
TRIGGER `cedar`.`jurisdiction_AFTER_DELETE`
AFTER DELETE ON `cedar`.`jurisdiction`
FOR EACH ROW
BEGIN
